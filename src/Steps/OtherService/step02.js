...
function Step02({ register, handleSubmit, watch, setValue, reset, clear}) {
  ...
  const answerRelate = () => {
    ...
    if (watch("StepTwoQuest01") === "sim") {
      relate[2] = "Conseguiu localizar a assinatura do cliente?  ( X ) Sim  (   ) Não\n";
      
      if(watch("StepTwoQuest02") === "Synsuite"){
        relate[2] += "Cliente com seu cadastro em:   ( X ) Synsuite  (   ) Outro Sistema\n";
      }else{
        relate[2] += "Cliente com seu cadastro em:   (   ) Synsuite  ( X ) Outro Sistema\n";
      }
    
    } else {
      relate[2] = "Conseguiu localizar a assinatura do cliente?  (   ) Sim  ( X ) Não\n";
    }


    if (watch("StepTwoQuest03") === "sim") {
      relate[3] = "Campo Telefone (para Ligações) atualizado?  ( X ) Sim  (   ) Não\n";
    }else{
      relate[3] = "Campo Telefone (para Ligações) atualizado?  (   ) Sim  ( X ) Não\nMotivo: "+watch("StepTwoQuest04")+"\n";
    }


    if (watch("StepTwoQuest05") === "sim") {
      relate[4] = "Campo Número (para WhatsApp) atualizado?  ( X ) Sim  (   ) Não\n";
    }else{
      relate[4] = "Campo Número (para WhatsApp) atualizado?  (   ) Sim  ( X ) Não\nMotivo: "+watch("StepTwoQuest06")+"\n";
    }
    ...
    if (watch("StepTwoQuest07") === "sim") {
      ...
      relate[5] = "E-mail no sistema atualizado?  (   ) Sim  ( X ) Não\nMotivo: "+watch("StepTwoQuest08")+"\n\n";
    }
